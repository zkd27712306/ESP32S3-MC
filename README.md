...
- 网络模式：默认 AP 热点模式，支持 Web 配置与 AP/STA 切换
...
| 🌐 网络 | 首次上电进 Web 配置，之后默认 AP 热点，支持 BOOT 键切换 AP/STA |
...
1. ESP32-S3 上电
2. 自动进入 **Web 配置模式**，创建名为 `ESP32-MC-Setup` 的 WiFi 热点
3. 密码：`12345678`
4. 用手机或电脑连上该热点
5. 浏览器打开 `192.168.4.1`
6. 填写 WiFi 信息并选择模式（STA 连接路由器，或 AP 自建热点）
7. 保存后 ESP32 自动重启

### 后续上电（已配置）

1. ESP32-S3 上电
2. 默认进入 **AP 热点模式**，创建名为 `ESP32-MC` 的 WiFi 热点
3. 密码：`ESP32-MC`
4. 服务器监听端口：`25565`
5. Minecraft Java 客户端连接 `192.168.4.1:25565`

> 如果之前配置为 STA 模式，则连接已保存的 WiFi，IP 由路由器分配。

### BOOT 键操作

| 操作 | 效果 |
|------|------|
| 长按 BOOT **2 秒** | 切换 AP / STA 模式，重启生效 |
| 长按 BOOT **10 秒** | 清空 WiFi 配置，重启后进入 Web 配置模式 |

### 连接 Minecraft

1. 打开 Minecraft Java 版 **26.1.2**
2. 进入 **多人游戏 → 添加服务器**
3. 地址填写：`192.168.4.1:25565`（AP 模式）
4. 点击 **加入服务器**
